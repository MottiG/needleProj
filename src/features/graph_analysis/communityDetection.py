...
# v_community_multilevel = pickle.load(open(os.path.join(graph_analysis_dir, 'v_community_multilevel' + '.pickle'), 'rb'))
t0 = time.time()
v_community_multilevel_list = G.community_multilevel(return_levels=True)
print(time.time() - t0)
t0 = time.time()
print("community_label_propagation")
v_community_label_propagation = G.community_label_propagation()
print(time.time() - t0)
# t0 = time.time()
# print("community_edge_betweenness")
# v_community_edge_betweenness = G.community_edge_betweenness()
# print(time.time() - t0)
...
# pickle.dump(v_community_edge_betweenness, open(os.path.join(graph_analysis_dir, 'v_community_edge_betweenness'+'.pickle'),'wb'))
pickle.dump(v_community_label_propagation, open(os.path.join(graph_analysis_dir, 'v_community_label_propagation'+'.pickle'),'wb'))
pickle.dump(v_community_multilevel_list, open(os.path.join(graph_analysis_dir, 'v_community_multilevel_list'+'.pickle'),'wb'))
