...
class GraphFeaturesBuilder:
    ...
    def community_vector_2sparse(self, v_community, minimal_community_size):
        ...
        for community in v_community:
            if len(community) >= minimal_community_size:
                community_graphID2rowID = [self.graphID2rowId[x] for x in community]
    ...
    def get_features(self) -> dict:
        ...
        community_multilevel_sparse = self.community_vector_2sparse(self.v_community_label_propagation[-1],minimal_community_size)
...
print("GraphFeaturesBuilder")
test = GraphFeaturesBuilder(df)

print("test.get_features()")
features_dict = test.get_features()

print("dump")
pickle.dump(test.community_layers_df , open(os.path.join(graph_analysis_dir, 'test.community_layers_df'+'.pickle'),'wb'))
pickle.dump(features_dict , open(os.path.join(graph_analysis_dir, 'test.community_layers_df'+'.pickle'),'wb'))
